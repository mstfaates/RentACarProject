...
export class PaymentComponent implements OnInit {
        ...
        .subscribe((response) => {
          ...
          if (card.moneyInTheCard < this.paymentAmount)
            return this.toastrService.error(
              'Kartınızda yeterli bakiye yoktur',
              'Hata'
            );
  ...
  async getCreditCardByCardNumber(cardNumber: string) {
    return (
      await this.creditCardService.getByCardNumber(cardNumber).toPromise()
    ).data[0];
  }
  ...
  updateFindexPointOfCurrentCustomer() {
    ...
    this.customerService
      .getCustomerByEmail(currentCustomer.email)
      .subscribe((response) => {
  ...
  saveCard(card: CreditCard) {
    this.creditCardService.add(card).subscribe(
      (response) => {
